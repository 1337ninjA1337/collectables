 ...
 *
 * Scope: INSERT events only. UPDATE/DELETE propagation is BE-19. Each helper is
 * a no-op (returns a stub) when Supabase is unconfigured, realtime is disabled
 * by the kill-switch, or `userId` is empty, so callers can wire them
 * unconditionally inside an effect.
 */
...
/**
 * Subscribe to INSERTs of the signed-in user's own collections
 * (`owner_user_id = userId`). Each new row is coerced via the same pure
...
/**
 * Subscribe to INSERTs of every item the signed-in user authored
