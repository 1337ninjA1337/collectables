...
import { byCreatedAtAscThenId } from "@/lib/sort-helpers";
...
export function ChatProvider({ children }: React.PropsWithChildren) {
  ...
  // nothing upstream holds it. See `lib/storage-notice.ts`.
  useStorageNotice(ready && !hydrationSafeToPersist && !!storageKey);
  ...
  const getMessages = useCallback(
    (chatId: string) => {
      ...
      return [...msgs].sort(byCreatedAtAscThenId);
    },
    [store],
  );
