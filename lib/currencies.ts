/**
 * Full ISO 4217 active currency list shared by the cost/currency pickers.
 * Keep this module pure (no React Native imports) so it can be unit-tested
 * in isolation and reused anywhere a currency selector is needed.
 */

export type Currency = {
  /** ISO 4217 alphabetic code, e.g. "USD". */
  code: string;
  /** English currency name, e.g. "US Dollar". */
  name: string;
};
...
/** True when `code` is a known ISO 4217 alphabetic code in {@link CURRENCIES}. */
export function isCurrencyCode(code: string): boolean {
  return CURRENCY_CODES.has(code);
}

/** Look up a currency by code, or `undefined` if it is not in the list. */
