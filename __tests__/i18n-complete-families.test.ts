...
const FAMILIES: readonly Family[] = [
  ...
  },
  {
    ...
    size: 28,
    because:
      "the screen behind the app icon, and the one every session starts and returns to — measured on 2026-08-22 as reading 37 base keys, 12 owned by prefix families and one (`appName`) exempt, leaving 25 of which 19 were not declared in all six locales. The most MIXED screen listed so far: `createCollection` was missing only in `de`, `myCollections` in three, `friends` in all four, so a German reader met a section header in German above two English tiles — which is the seam a screen family closes and a per-key gap count never showed. 27 since 2026-09-10, when `moveUp` and `moveDown` arrived together as the screen-reader route into reordering owned collections — both translated in all six locales in the same commit, which is what this number is here to force. 29 the same day, when `reorderPickedUp` and `reorderMoved` gave the mode a voice — the two announcements are the only keys on this screen a user never reads. Back to 27 later the same day: `moveUp` and `moveDown` are no longer NAMED here, because the twelve lines of action wiring both reorder screens duplicated became `reorderActionProps` in `lib/reorder-actions.ts`, which is handed `t` and asks for its own labels. The keys did not leave the app — `lint:orphan-i18n` still reaches them through that module, and `keyboard-reorder-actions.test.ts` still requires both in all six locales — they left this FILE, which is what a screen family counts. 26 an hour later, when the long press joined them: the pick-up announcement is `reorderActionProps`'s now, so `reorderPickedUp` is no longer named here either — `reorderMoved` still is, because the DRAG's own landing announcement stayed in the screen. 29 on 2026-09-11, when the friends and subscribed tabs stopped mounting every card they were handed: `loadMoreItems`, `loadMoreItemsA11y` and `loadMoreItemsHint` arrived already translated in all six locales, because `app/collections-feed.tsx` has rendered the same CTA over the same two lists since it was written — a key family joining a screen without a translation round is what this number is supposed to let through, and the only reason it can is that the earlier screen paid for it. Back to 26 the same day, and so is collection detail, three lower: the button those keys belong to was written out on three screens, and `<LoadMoreButton>` took the markup, the styles and the strings. Same movement as `reorderActionProps` — the keys did not leave the app, `lint:orphan-i18n` still reaches them, they left these FILES. 28 on 2026-09-12: `archiveTitle` and `archiveHint`, the conditional row pointing at the trash — the archive is the first thing this screen links to that a user reaches only after a mistake, and a recovery path served in English under somebody else's flag is the one place the fallback costs the most",
  },
  {
    ...
    size: 55,
    because:
      "the screen a collector actually lives in — the one the 'this screen is half-translated' complaint was originally about — and the largest single holder left on 2026-08-22 at 27 of its 46 keys English in all four partial locales. It is also the first listed screen whose keys are not all its own: it renders through `<ItemCard>`, `<BulkBar>`, `<EditCollectionModal>`, the share sheet and more, so this entry finishes the FILE while a user reads the union, and the components are listed separately in their own suites. 48 since 2026-09-10, when `moveUp` and `moveDown` reached the item rows — the same two labels the home screen took two commits earlier, which is what a screen family counts and a per-key gap never would. 50 the same day, with the two reorder announcements; back to 48 later the same day, when the action wiring both screens duplicated moved into `lib/reorder-actions.ts` and took the two label keys with it — still translated everywhere, still read by that module, no longer named in this file. 47 an hour later, when the long press moved there too and took `reorderPickedUp` with it. 49 on 2026-09-10, when entering reorder mode started CLEARING an active sort rather than explaining why dragging was off: `sortClearedForReorder` says what happened and `undo` puts it back. 50 the same day, with `sortRestored` — the undo had to be audible too. 47 on 2026-09-11, when the drag fallback's Load-more CTA became `<LoadMoreButton>`: this screen had the first copy of that button and the home screen had just added the third, so the three `loadMore*` keys went with the markup to the component that now renders all of them. 52 on 2026-09-12, when the bulk bar gained a reversible resolution: `itemsArchived`, `archiveItemsTitle`, `archiveItemsText` and `bulkListedWarning` are new, and `archiveAction` is the fifth — the screen labels the confirm's button with the same word the bar does. Five for a four-key round is what a screen family counts. 54 later the same day: `exportTotalUnconverted` and `exportTotalPartial`, the two sentences the exported PDF's total gains when it does not mean what it looks like — the rate table was missing, or it was missing a currency. This screen resolves which one it is because the builder takes strings and the counted sentence needs a number. 55 later still: `bulkListingsRemoved`, the half of a bulk outcome the toasts left out — the confirms counted listings and the toasts counted only rows, so a seller who archived thirty and withdrew four heard about the thirty",
  },
  {
    ...
    size: 51,
    because:
      "the screen a collector opens most often once they own anything, and the one where the app finally shows a person their own stuff — measured on 2026-08-22 as reading 59 base keys, 21 of them already owned by prefix families, leaving 38 of which 12 were English in all four partial locales. Five of those twelve (`delete`, `acquiredHow`, `acquiredDate`, `variants`, `share`) are also rendered by `app/collection/[id].tsx`, which is the next screen and the largest remaining holder. 39 on 2026-09-10, with `tagsDuplicate` — its edit mode IS the add-item form, so the two families took the key together. 46 on 2026-09-12, when archiving finally reached the screen the item is on: four new keys (`archiveAction`, `archiveActionDone`, `archiveBannerTitle`, `archiveBannerHint`) plus three the archive screen had already paid for (`archiveRestore`, `archiveRestored`, `undo`). Seven for a four-key round is what a screen family counts and a per-key diff would not show — and it is the right number, because a reader of this screen meets all seven. 49 later the same day, when archiving started taking an open listing down with it: `archiveListedTitle` and `archiveListedBody` (the confirm that says the listing does not come back) and `archiveActionDoneListingRemoved` (the toast for the outcome the plain one does not describe). 50 later still, with `deleteItemListedText`: deleting a listed item takes the listing too, and a confirm naming only the item was describing half of what the button does on the one action nothing can undo. Still 50 on 2026-09-12, when `archiveActionDoneListingRemoved` was retired and the branch started composing `archiveActionDone` with `bulkListingsRemoved` instead — a key left the base vocabulary and this family did not shrink, because the screen still renders a sentence about the listing and now renders the one the bulk bar already had. 51 later the same day, with `archiveDeleted`: deleting one item from this screen said nothing at all, so a seller who deleted a listed item withdrew a standing offer from every buyer's device and was told nothing — on the one action in the family nothing can undo. The sentence is the one `app/archive.tsx` already says for the same act, so the family grew by a key that cost no translation",
  },
  {
    name: "sign-in screen",
