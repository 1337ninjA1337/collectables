...
async function bootEnabled() {
  ...
  await initAnalytics({
    ...
    loader: async () => makeFakeCtor(fake) as never,
  });
  return fake;
}
...
describe("Analytics #17 — trackEvent rate limiter", () => {
  ...
  it("__resetAnalyticsRateLimitForTests clears the window", async () => {
    const { calls } = await bootEnabled();
    for (let i = 0; i < 200; i += 1) trackEvent("chat_opened");
  ...
  it("__resetAnalyticsForTests also clears the rate-limit window", async () => {
    let fake = await bootEnabled();
    for (let i = 0; i < 200; i += 1) trackEvent("language_switched");
    assert.equal(fake.calls.length, 200);
    // A full reset (re-boot) must not carry the exhausted window over.
    fake = await bootEnabled();
