...
describe("app/collection/[id].tsx — HM-A header-fragment memoization", () => {
  it("listTitleAndFilters is a useMemo with honest deps", () => {
    ...
    // (empty-state gate), the filter state, the writer that also remembers the
    // sort (`applyFilters`, a useCallback and NOT React-stable), the
    // reorder-blocked-by-sort flag and its reset handler (the sorted-owner
    // notice), and the translator.
    assert.match(
      src,
      /listTitleAndFilters\s*=\s*useMemo\([\s\S]*?\[allItems\.length,\s*applyFilters,\s*itemFilters,\s*reorderBlockedBySort,\s*resetSort,\s*t\],?\s*\n\s*\)/,
      "listTitleAndFilters deps must be [allItems.length, applyFilters, itemFilters, reorderBlockedBySort, resetSort, t]",
    );
  });
