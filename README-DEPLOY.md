...
| `EXPO_PUBLIC_SENTRY_RELEASE`                 | `EXPO_PUBLIC_SENTRY_RELEASE`               | Optional. Overrides the Sentry release identifier. Highest precedence — CI usually sets it to the commit SHA so crashes map to an exact build. When empty, falls back to `EXPO_PUBLIC_APP_VERSION`, then `collectables@<app.json version>`. |
| `EXPO_PUBLIC_APP_VERSION`                    | `EXPO_PUBLIC_APP_VERSION`                  | Optional. Feeds the Sentry release as `collectables@<value>` when `EXPO_PUBLIC_SENTRY_RELEASE` is unset. When both are empty, the `app.json` `expo.version` is used. |
