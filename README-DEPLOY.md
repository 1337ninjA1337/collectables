...
| `EXPO_PUBLIC_SENTRY_TRACES_SAMPLE_RATE`      | `EXPO_PUBLIC_SENTRY_TRACES_SAMPLE_RATE`    | Optional. Performance-tracing sample rate in `[0, 1]`. Defaults to `0.1`; unset/non-numeric/out-of-range values fall back to the default. Tune production sampling without a redeploy. |
| `EXPO_PUBLIC_SENTRY_RELEASE`                 | `EXPO_PUBLIC_SENTRY_RELEASE`               | Optional. Overrides the Sentry release identifier. Highest precedence — CI usually sets it to the commit SHA so crashes map to an exact build. When empty, falls back to `EXPO_PUBLIC_APP_VERSION`, then `collectables@<package.json version>`. |
| `EXPO_PUBLIC_APP_VERSION`                    | `EXPO_PUBLIC_APP_VERSION`                  | Optional. Feeds the Sentry release as `collectables@<value>` when `EXPO_PUBLIC_SENTRY_RELEASE` is unset. When both are empty, `package.json`'s `version` is auto-resolved at build time (`app.config.ts` → `lib/app-version.ts`), so this var is normally unnecessary. |
| `EXPO_PUBLIC_SENTRY_SOURCEMAPS`              | — (workflow-derived)                       | Optional. Set to `1` by the deploy workflow when `SENTRY_AUTH_TOKEN` was present at build time, i.e. the sourcemap-upload step ran. Powers the `getSentryStatus().sourcemapsExpected` diagnostics hint ("events flowing but stack traces minified"). Never gates event capture — runtime needs only the DSN. |
...
| `EXPO_PUBLIC_SIGNUP_FRESHNESS_WINDOW_MS`     | `EXPO_PUBLIC_SIGNUP_FRESHNESS_WINDOW_MS`   | Optional. Freshness window (ms) for detecting a just-created account when firing the `signup_completed` analytics event. Defaults to `300000` (5 min). Widen (e.g. `3600000` = 60 min) on staging deploys where a slow `verifyOtp` round-trip may push the check past the default window. Unset/non-numeric/non-positive values fall back to the default. |
...
| `EXPO_PUBLIC_ANALYTICS_MIRROR_DISABLED`      | `EXPO_PUBLIC_ANALYTICS_MIRROR_DISABLED`    | Optional. Set to `1`, `true`, or `yes` to flip `AnalyticsConfig.mirrorDisabled` to `true` — any client-side wiring of the PostHog→Supabase `analytics-mirror` webhook must treat the long-tail event mirror as off, while PostHog and Clarity themselves stay live. Narrower than `EXPO_PUBLIC_ANALYTICS_DISABLED` (which implies it). Useful for incident response (drop the Supabase event store without redeploying the Edge Function) and Posthog-only QA. |
| `EXPO_PUBLIC_ANALYTICS_ENV`                  | `EXPO_PUBLIC_ANALYTICS_ENV`                | Optional. One of `development`, `staging`, `production`; overrides `EXPO_PUBLIC_SENTRY_ENV` for analytics only. `development` disables analytics reporting entirely. When unset, the analytics environment follows `EXPO_PUBLIC_SENTRY_ENV`, then defaults to `production`. |
...
| `EXPO_PUBLIC_ALLOW_RUNTIME_SUPABASE_CONFIG`  | `EXPO_PUBLIC_ALLOW_RUNTIME_SUPABASE_CONFIG`| Optional. **Leave UNSET for production.** Set to the literal `true` only on a QA/preview deploy to let the app read its Supabase URL/key from `localStorage` (`collectables-supabase-runtime-config`) over the build-time secrets. Off by default (and in `__DEV__` dev builds it is always on) — SEC-4 closes the account-takeover vector where XSS / a malicious extension could repoint a production build at an attacker's Supabase project. |
