...
| `listing_claimed` | `mode`, `sellerWasFriend`, `sellerRelationship` | Fired from `app/listing/[id].tsx` after the buy/trade flow completes. `sellerWasFriend` measures the social-graph contribution to marketplace velocity; `sellerRelationship` is the finer friend/following/stranger bucket (`relationshipForAnalytics`) so reports can slice friend trades from stranger sales. |
...
| `listing_dropped` | `mode`, `hasPrice` | Fired from `app/item/[id].tsx` when the listing sheet is dismissed with a dirty draft (the user filled in fields but never published). The abandon arm balancing `listing_created`; `hasPrice` flags whether a price had been typed at dismissal. |
| `listing_price_invalid` | `reason`, `language` | Fired from `app/item/[id].tsx` when a sell-mode submit is rejected because the typed price fails `parseCurrencyValue`. `reason` classifies the failure (empty / unparseable / non_positive); `language` surfaces locales where comma-vs-dot decimal habits drive high invalid rates. |
| `listing_view` | `mode`, `sellerRelationship`, `isSold` | Fired from `app/listing/[id].tsx` after a dwell-time gate (`useDwellTimeEffect`) when a user views someone else's listing — pagination scroll-pasts and navigation flickers never count. The denominator for `listing_claimed`'s view-to-buy conversion; `sellerRelationship` slices conversion by social proximity, `isSold` separates browsing the active feed from viewing sold history. |
| `premium_activated` | `source` | Fired from the premium false→true transition hook in `components/bottom-nav.tsx`. `source` carries which screen triggered the upgrade via the one-shot `consumeLastPremiumIntent()` intent ref (`settings` / `create_collection` / `upsell_sheet` / `server_sync` for entitlements restored by the cloud validation merge / `unknown` for an untagged caller). |
| `premium_upsell_shown` | `feature`, `source`, `control` | Fired when a free user hits a premium gate (e.g. taps the locked Private visibility chip). `feature` names the gated capability, `source` the screen, and `control` which affordance was pressed — `chip` for the padlocked chip, `hint` for the sentence beside it. Zero events for a feature over a long window is the evidence needed to hide its locked affordance entirely, and that decision is per-CONTROL: the two send the same feature and source, so without `control` a tap on the sentence reads as a tap on the chip. |
