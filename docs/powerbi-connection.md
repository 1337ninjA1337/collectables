...
| `premium_upsell_shown` | `feature`, `source` | Fired when a free user hits a premium gate (e.g. taps the locked Private visibility chip). `feature` names the gated capability, `source` the screen. Zero events for a feature over a long window is the evidence needed to hide its locked affordance entirely. |
