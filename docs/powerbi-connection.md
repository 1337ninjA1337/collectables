...
`docs/powerbi/Collectables-Starter.pbit` ships in the repo (generated by
`npm run build:powerbi` from the verifiable
[`docs/powerbi/`](./powerbi/) text source assets). Open it in Power BI
Desktop and it prompts for the four `Supabase*` parameters, then loads the
`analytics_events` table with the DAU / listing-funnel / premium-conversion
measures already defined — no DAX/M authoring.

If your Power BI build rejects the hand-authored template, fall back to the
copy-paste path in [`docs/powerbi/README.md`](./powerbi/README.md)
(`queries.m` + `measures.dax`) — that is the same source the `.pbit` is
generated from, so the result is identical. Dashboard screenshots will be
added here once a reference report is published.
